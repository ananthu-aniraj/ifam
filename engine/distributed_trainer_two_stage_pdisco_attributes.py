...
class PDiscoTrainerTwoStageAttributes:
    def __init__(
            self,
        ...
        self.total_num_attributes = model.total_num_attributes
    ...
    def _init_loss_dict(self) -> None:
        self.loss_dict_train = {'loss_attribute_prediction_train': AverageMeter(),
    ...
    def _init_accuracy_metrics(self) -> None:
        self.acc_dict_train = {
            'attribute_prediction_acc_train': MultilabelAccuracy(num_labels=self.total_num_attributes,
                                                                 average='macro').to(self.local_rank,
                                                                                     non_blocking=True),
            'attribute_prediction_ap_train': MultilabelAveragePrecision(
                num_labels=self.total_num_attributes,
                average='macro',
                thresholds=None).to(self.local_rank,
                                    non_blocking=True),
            'attribute_prediction_auroc_train': MultilabelAUROC(
                num_labels=self.total_num_attributes,
                average='macro', thresholds=None).to(self.local_rank,
                                                     non_blocking=True)}

        self.acc_dict_test = {'attribute_prediction_acc': MultilabelAccuracy(num_labels=self.total_num_attributes,
                                                                             average='macro').to(self.local_rank,
                                                                                                 non_blocking=True),
    ...
    def _run_batch(self, source, targets, train: bool = True, vis_att_maps: bool = False, curr_iter: int = 0) -> \
        ...
        with torch.set_grad_enabled(train), torch.amp.autocast(device_type="cuda", dtype=torch.float16,
            ...
            outputs, maps = self.model(source)
            ...
            if train:
                ...
                loss_attribute_prediction = self.loss_fn_train(outputs, targets) * self.l_classification
                ...
                loss = loss_conc + loss_presence + loss_attribute_prediction + loss_equiv + loss_tv + loss_enforced_presence + loss_pixel_wise_entropy
                ...
                losses_dict = {'loss_attribute_prediction_train': loss_attribute_prediction.item(),
            ...
            if it % self.log_freq == 0:
                if train:
                    print(
                        ...
                        f'| Attribute Prediction Loss {losses_dict["loss_attribute_prediction_train"]:.5f} | Concentration Loss {losses_dict["loss_conc_train"]:.5f}'
    ...
    def test_only(self):
        ...
        with torch.inference_mode():
            if self.test_loader:
                ...
                print(
                    f'Test loss: {loss_dict_val["loss_total_val"]:.5f} | Attribute prediction acc: {acc_dict_test["attribute_prediction_acc"]:.5f}'
...
def launch_pdisco_2_stage_attributes_trainer(model: torch.nn.Module,
                                             train_dataset: torch.utils.data.Dataset,
                                             test_dataset: torch.utils.data.Dataset,
                                             batch_size: int,
                                             optimizer: torch.optim.Optimizer,
    ...
    model_trainer = PDiscoTrainerTwoStageAttributes(model=model, train_dataset=train_dataset, test_dataset=test_dataset,
                                                    ...
                                                    log_freq=log_freq,
                                                    use_amp=use_amp,
                                                    grad_norm_clip=grad_norm_clip, max_epochs=epochs,
                                                    num_workers=num_workers,
