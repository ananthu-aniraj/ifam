...
class BaselineAttributeTrainer:
    def __init__(
            self,
            ...
            use_ddp: bool = False,
            probe_specific_patch: bool = False,
    ) -> None:
        self._init_ddp(use_ddp)
        self.total_num_attributes = model.num_classes
        self.probe_specific_patch = probe_specific_patch
        ...
        self.fmap_size = self.h_fmap * self.w_fmap
    ...
    def _init_acc_attr(self) -> None:
        self.acc_dict_test_attr = {'attribute_prediction_acc': MultilabelAccuracy(num_labels=self.total_num_attributes,
    ...
    def _run_batch(self, source, targets, part_locs, train: bool = True) -> Tuple[Any, Any]:
        with torch.set_grad_enabled(train), torch.amp.autocast(device_type="cuda", dtype=torch.float16,
                                                               enabled=self.use_amp):

            outputs = self.model(source, part_locs)
    ...
    def _run_epoch(self, epoch: int, dataloader: DataLoader, train: bool = True):
        ...
        for key in self.acc_dict_test_attr.keys():
            self.acc_dict_test_attr[key].reset()

        for it, batch_data in enumerate(dataloader):
            step_type = "Train" if train else "Eval"
            source = batch_data[0].to(self.local_rank, non_blocking=True)
            targets = batch_data[1].to(self.local_rank, non_blocking=True)
            part_locs = batch_data[2].to(self.local_rank, non_blocking=True)

            # Remove images where part_locs go out of bounds
            if self.pooling_type == "specific_patch_probe":
                token_loc = ((part_locs[:, 0] * self.h_fmap) * (part_locs[:, 1] * self.w_fmap)).long()  # (B,)
                token_loc = token_loc - 1  # 0-indexed

                valid_idx = (token_loc >= 0) & (token_loc < self.fmap_size)
                source = source[valid_idx]
                targets = targets[valid_idx]
                part_locs = part_locs[valid_idx]
            if train and self.mixup_fn is not None:
                source, targets = self.mixup_fn(source, targets)
            batch_preds, batch_loss = self._run_batch(source, targets, part_locs, train)
            if train:
                num_updates += 1
                ...
                for key in self.acc_dict_test_attr.keys():
                    self.acc_dict_test_attr[key].update(batch_preds, targets.long())
        ...
        if train:
            ...
            for key in self.acc_dict_test_attr.keys():
                accuracies_dict[key] = self.acc_dict_test_attr[key].compute().item() * 100
...
def launch_attribute_trainer(model: torch.nn.Module,
                             train_dataset: torch.utils.data.Dataset,
                             test_dataset: torch.utils.data.Dataset,
                             batch_size: int,
                             optimizer: torch.optim.Optimizer,
    ...
    if use_ddp:
        ddp_setup()
    trainer = BaselineAttributeTrainer(model=model, train_dataset=train_dataset, test_dataset=test_dataset,
                                       ...
                                       probe_specific_patch=probe_specific_patch)
    if eval_only:
        trainer.test_only()
    else:
        trainer.train()
