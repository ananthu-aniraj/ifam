...
from data_sets import FineGrainedBirdClassificationDataset, CelebA, ImageNetWithOODEval, PartImageNetDataset, PlantNet, \
    CUBAttributesDataset, PlantCLEF2024, CUBPartAttributesDataset, CXRDataset, WaterBirdsDataset, NABirdsCUBSubset
...
def get_dataset(args, train_transforms, test_transforms):
    if args.dataset == 'cub':
    ...
    elif args.dataset == 'cub_attr_only':
        dataset_train = CUBAttributesDataset(args.data_path, split=args.train_split, mode='train',
                                             transform=train_transforms,
                                             image_sub_path=args.image_sub_path_train,
                                             use_image_level_attributes=args.use_image_level_attributes,
                                             load_non_part_attributes=args.load_non_part_attributes)

        dataset_test = CUBAttributesDataset(args.data_path, mode=args.eval_mode,
                                            transform=test_transforms, image_sub_path=args.image_sub_path_test,
                                            use_image_level_attributes=args.use_image_level_attributes,
                                            load_non_part_attributes=args.load_non_part_attributes)
        num_cls = dataset_train.total_num_attributes

    elif args.dataset == 'cub_attr':
        dataset_train = CUBPartAttributesDataset(args.data_path, split=args.train_split, mode='train',
                                                 transform=train_transforms,
                                                 image_sub_path=args.image_sub_path_train,
                                                 use_image_level_attributes=True,
                                                 load_non_part_attributes=args.predict_non_part_attributes,
                                                 relevant_part_name=args.part_to_probe)
        dataset_test = CUBPartAttributesDataset(args.data_path, mode=args.eval_mode,
                                                transform=test_transforms,
                                                image_sub_path=args.image_sub_path_test,
                                                use_image_level_attributes=True,
                                                load_non_part_attributes=args.predict_non_part_attributes,
    ...
    elif args.dataset == 'na_birds_cub_subset':
        dataset_train = NABirdsCUBSubset(args.data_path, transform=train_transforms, image_sub_path=args.image_sub_path_train)
        dataset_test = NABirdsCUBSubset(args.data_path, transform=test_transforms, image_sub_path=args.image_sub_path_test)
        num_cls = dataset_train.num_classes
        ...
        dataset_test = PlantNet(args.data_path, args.image_sub_path_test, transform=test_transforms,
                                ...
                                species_id_to_name_file=args.species_id_to_name_file)
        num_cls = dataset_test.num_classes
    elif args.dataset == 'plantclef2024':
        dataset_train_p1 = PlantCLEF2024(args.data_path, args.image_sub_path_train, transform=train_transforms)
        dataset_train_p2 = PlantCLEF2024(args.data_path, 'val', transform=train_transforms)
        dataset_train = torch.utils.data.ConcatDataset([dataset_train_p1, dataset_train_p2])
        train_cls_num_list = [dataset_train_p1.cls_num_list[idx] + dataset_train_p2.cls_num_list[idx] for idx in
                              range(len(dataset_train_p1.cls_num_list))]
        dataset_train.cls_num_list = train_cls_num_list
        dataset_test = PlantCLEF2024(args.data_path, args.image_sub_path_test, transform=test_transforms)
        num_cls = dataset_train_p1.num_classes
...
def get_dataset_with_attributes(args, train_transforms, test_transforms):
    if args.dataset == 'cub':
        dataset_train = CUBAttributesDataset(args.data_path, split=args.train_split, mode='train',
                                             transform=train_transforms,
                                             image_sub_path=args.image_sub_path_train,
